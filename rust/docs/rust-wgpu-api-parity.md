...
| `BufferUsage`        | Done              | Partial     | Partial          | Rust exposes usage preset functions such as `BufferUsage::vertex()`. Wasm exposes the JS-style `BufferUsage.vertex` constant; additional usage constants are pending.                    |
| `Mesh`               | Done              | Done        | Partial          | Rust supports vertex layout generation, slot assignment, index metadata, and render-pass binding. Wasm supports chained `Mesh.addVertexBuffer`; index and bind-group parity are pending. |
