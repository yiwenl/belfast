...
- The complete Cargo workspace lives at the repository root (`crates/`), while the TypeScript package is frozen under `archive/`.
...
- A public API parity matrix should be maintained as part of the feature documentation: a hand-maintained markdown table in `docs/rust-wgpu-api-parity.md` with one row per public export of `archive/packages/belfast/src/index.ts` and columns for TypeScript, Rust native, WebAssembly, and notes.
