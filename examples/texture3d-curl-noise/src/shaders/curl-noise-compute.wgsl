...
fn cs_main(@builtin(global_invocation_id) globalId: vec3<u32>) {
  ...
  let worldPos = uvw * 4.0 - 2.0;
  let samplePos = worldPos * params.curlScale + vec3(params.time * 0.15);
  let dir = curlNoise(samplePos);
  let mag = length(dir);

  textureStore(velocityOut, globalId, vec4<f32>(dir, mag));
}
